...
const Specials: React.FC = () => {
  return (
    <section id="specials" className="py-20 bg-stone-100">
      <div className="container mx-auto px-6">
        ...
        >
            <h2 className="text-4xl font-serif font-bold text-center mb-4 text-amber-900">Today's Specials</h2>
        ...
        <div className="grid md:grid-cols-2 gap-12 items-center">
          {SPECIALS_DATA.map((item, index) => (
            <motion.div 
                key={item.id}
                className="flex flex-col md:flex-row gap-6 bg-white p-6 rounded-lg shadow-xl"
                initial={{ opacity: 0, x: index % 2 === 0 ? -50 : 50 }}
                whileInView={{ opacity: 1, x: 0 }}
                viewport={{ once: true, amount: 0.5 }}
                transition={{ duration: 0.5, delay: 0.2 * index }}
            >
              <img src={item.image} alt={item.name} className="w-full md:w-48 h-48 object-cover rounded-md" />
              <div>
